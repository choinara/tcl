...
interface UserMemberFilters {
  keyword: string;
  status: string;
}

interface UserMemberFilterFormProps {
  onSearch: (filters: UserMemberFilters) => void;
}
...
export const UserMemberFilterForm = ({ onSearch }: UserMemberFilterFormProps) => {
  ...
  const update = (key: keyof UserMemberFilters, value: string) => setFilters((prev) => ({ ...prev, [key]: value }));
      ...
      <div className="flex items-stretch w-full">
        <div className="flex flex-col gap-2.5 flex-1 px-3 py-2.5">
          <div className="flex gap-5 items-center">
            <Input value={filters.keyword} onChange={(val: string) => update('keyword', val)} placeholder="이름 또는 사용자ID로 검색" heightType="h32" style={{ width: 300 }} onKeyDown={(e: KeyboardEvent) => e.key === 'Enter' && handleSearch()} />
