...
const emptyForm = (): MaintenanceRow => ({
  isClosed: 'N',
  failDate: new Date().toISOString().split('T')[0],
});

export default function EquipMaintenancePage() {
  const { t } = useTranslation();
  const perm = usePermission('ET0010');
  ...
  const gridRef = useRef<AgGridReact<MaintenanceRow>>(null);

  const [filterClosed, setFilterClosed] = useState<string>('');
  const [keyword, setKeyword] = useState('');
  const [showForm, setShowForm] = useState(false);
  const [editRow, setEditRow] = useState<MaintenanceRow>(emptyForm());
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await authFetch(`/api/et/maintenance?${params}`);
      if (res.ok) {
        const json = await res.json();
        setRows(json.data?.content || []);
      }
    } catch {
      notify(t('message.networkError', '보전이력 조회에 실패했습니다'), { type: 'error' });
    }
  ...
  const columns = useMemo<ColDef<MaintenanceRow>[]>(() => [
    ...
    { field: 'failTypeCode', headerName: '고장유형', width: 100, valueFormatter: (p) => {
      const c = failTypesRef.current.find((f: CommonCode) => f.code === p.value);
    ...
    { field: 'repairCost', headerName: '수리비용', width: 110 },
    ...
    { field: 'shiftCode', headerName: '조', width: 80, valueFormatter: (p) => {
      const c = shiftCodesRef.current.find((s: CommonCode) => s.code === p.value);
      return c ? c.codeName : String(p.value ?? '');
    }},
    { field: 'failDesc', headerName: '고장내용', flex: 1, minWidth: 150 },
    { field: 'repairDesc', headerName: '수리내용', flex: 1, minWidth: 150 },
    { field: 'remark', headerName: '비고', width: 120 },
  ], []);
  ...
  const handleEdit = (row: MaintenanceRow) => { setEditRow({ ...row }); setIsNew(false); setShowForm(true); };
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const url = isNew ? '/api/et/maintenance' : `/api/et/maintenance/${editRow.id}`;
  ...
  const handleDelete = useCallback(async (row: MaintenanceRow) => {
    if (!row.id) return;
    if (!await confirmDialog(`보전이력 ${row.repairNo ?? ''}을 삭제하시겠습니까?`)) return;
    try {
      const res = await authFetch(`/api/et/maintenance/${row.id}`, { method: 'DELETE' });
  ...
  return (
    <>
      <PageFilterShell
        ...
        toolbar={
          <>
            <select value={filterClosed} onChange={e => setFilterClosed(e.target.value)}
              className="mes-btn" style={{ fontSize: 12, padding: '4px 8px' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <tbody>
            <tr>
              ...
              <td style={{ padding: '4px 8px' }}>
                <select
                  value={editRow.failTypeCode ?? ''}
                  onChange={e => setEditRow(r => ({ ...r, failTypeCode: e.target.value }))}
                  style={{ width: '100%' }}
                >
                  <option value="">-- 선택 --</option>
                  {failTypes.map((c: CommonCode) => <option key={c.code} value={c.code}>{c.codeName}</option>)}
                </select>
              </td>
            </tr>
            <tr>
              ...
              <td style={{ padding: '4px 8px' }}>
                <select value={editRow.shiftCode ?? ''} onChange={e => setEditRow(r => ({ ...r, shiftCode: e.target.value }))} style={{ width: '100%' }}>
                  <option value="">-- 선택 --</option>
                  {shiftCodes.map((c: CommonCode) => <option key={c.code} value={c.code}>{c.codeName}</option>)}
                </select>
              </td>
            </tr>
            <tr>
