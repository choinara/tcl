...
interface MttrResponse {
  ...
  mttrValues: { equipId: number; mttrMin: number; failCount: number; totalLossMin: number }[];
  ucl: number;
}
interface MttrSearchParams { startDate: string; endDate: string; equipCategoryCode: string }

export default function EquipMttrPage() {
  const { t } = useTranslation();
  const perm = usePermission('ET0080');
  void perm;
  ...
  const [searchParams, setSearchParams] = useState<MttrSearchParams>({ startDate, endDate, equipCategoryCode });

  const handleSearch = useCallback(() => {
    setSearchParams({ startDate, endDate, equipCategoryCode });
  }, [startDate, endDate, equipCategoryCode]);

  const { data, isFetching, error } = useQuery<MttrResponse>({
    queryKey: ['et-mttr', searchParams.startDate, searchParams.endDate, searchParams.equipCategoryCode],
    queryFn: async ({ signal }) => {
      ...
      const res = await authFetch(`/api/et/loss/analysis/mttr?${params}`, { signal });
      if (!res.ok) throw new Error('MTTR 분석 조회 실패');
      const json = await res.json();
      return json.data as MttrResponse;
    },
    refetchOnWindowFocus: false,
  });

  useEffect(() => {
    if (error) notify(t('message.networkError', 'MTTR 분석 조회에 실패했습니다'), { type: 'error' });
  }, [error, notify, t]);

  const ucl = data?.ucl ?? 60;

  const chartData = useMemo(() =>
    (data?.equipment ?? []).map((eq, i) => ({
      name: `${eq.unitNumber}-${eq.lineName}`,
      value: data?.mttrValues[i]?.mttrMin ?? 0,
    })), [data]);

  const tableColDefs = useMemo<ColDef[]>(() => [
    { headerName: '설비', field: 'equipName', width: 130 },
    { headerName: 'MTTR(분)', field: 'mttrMin', width: 120, type: 'numericColumn' },
  ...
  const tableRows = useMemo(() =>
    (data?.equipment ?? []).map((eq, i) => {
      const mv = data?.mttrValues[i];
      const mttrMin = mv?.mttrMin ?? 0;
      return {
