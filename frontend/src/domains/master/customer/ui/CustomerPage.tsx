...
export default function CustomerPage() {
  const perm = usePermission('MM0010');
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await authFetch('/api/master/customers');
      if (res.ok) {
      ...
      notify('거래처 조회에 실패했습니다', { type: 'error' });
    }
  }, []);
  ...
  const columns = useMemo<ColDef[]>(() => [
    { field: 'customerCode', headerName: '고객코드', width: 150, editable: (p) => !p.data?.id },
    { field: 'customerName', headerName: '고객명', width: 200, editable: true },
    { field: 'location', headerName: '위치', width: 120, editable: true },
  ...
  const handleBatchSave = useCallback(async (rows: { _rowState: string; [key: string]: unknown }[]) => {
    const res = await authFetch('/api/master/customers/batch', {
      method: 'POST',
