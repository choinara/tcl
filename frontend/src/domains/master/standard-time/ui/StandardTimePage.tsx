...
export default function StandardTimePage() {
  const perm = usePermission('MM0030');
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await authFetch('/api/master/standard-times');
      if (res.ok) {
      ...
      notify('표준시간 조회에 실패했습니다', { type: 'error' });
    }
  }, []);
  ...
  const columns = useMemo<ColDef[]>(() => [
    {
      ...
      spanRows: (params: { valueA: unknown; valueB: unknown }) =>
        !!params.valueA && params.valueA === params.valueB,
    },
    { field: 'itemName', headerName: '항목', width: 250, editable: true },
    { field: 'standardHours', headerName: '표준시간(H)', width: 150, editable: true, cellDataType: 'number' },
  ...
  const handleBatchSave = useCallback(async (rows: { _rowState: string; [key: string]: unknown }[]) => {
    const res = await authFetch('/api/master/standard-times/batch', {
      method: 'POST',
