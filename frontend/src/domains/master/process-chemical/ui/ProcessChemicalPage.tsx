...
export default function ProcessChemicalPage() {
  const perm = usePermission('MM0040');
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await authFetch('/api/master/process-chemicals');
      if (res.ok) {
      ...
      notify('공정약품 조회에 실패했습니다', { type: 'error' });
    }
  }, []);
  ...
  const columns = useMemo<ColDef[]>(() => [
    ...
    { field: 'processName', headerName: '공정', width: 200, editable: true },
    { field: 'productName', headerName: '품명', width: 300, editable: true },
  ...
  const handleBatchSave = useCallback(async (rows: { _rowState: string; [key: string]: unknown }[]) => {
    const res = await authFetch('/api/master/process-chemicals/batch', {
      method: 'POST',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <PeakEditGrid
        ref={editGridRef}
        gridId="master-process-chemical"
        columns={columns}
        data={displayData}
