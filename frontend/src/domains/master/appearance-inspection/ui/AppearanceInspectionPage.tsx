...
export default function AppearanceInspectionPage() {
  const perm = usePermission('MM0110');
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await authFetch('/api/master/appearance-inspections');
      if (res.ok) {
      ...
      notify('외관검사 조회에 실패했습니다', { type: 'error' });
    }
  }, [notify]);
  ...
  const columns = useMemo<ColDef[]>(() => [
    { field: 'defectName', headerName: '불량명', width: 200, editable: true },
    { field: 'category', headerName: '구분', width: 150, editable: true },
    { field: 'subCategory', headerName: '세부구분', width: 200, editable: true },
    { field: 'requirement', headerName: '요구사항', width: 250, editable: true },
  ...
  const handleBatchSave = useCallback(async (rows: { _rowState: string; [key: string]: unknown }[]) => {
    const res = await authFetch('/api/master/appearance-inspections/batch', {
      method: 'POST',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <PeakEditGrid
        ref={editGridRef}
        gridId="master-appearance-inspection"
        columns={columns}
        data={displayData}
