...
export default function SupplierPage() {
  const perm = usePermission('MM0020');
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await authFetch('/api/master/suppliers');
      if (res.ok) {
      ...
      notify('업체 조회에 실패했습니다', { type: 'error' });
    }
  }, []);
  ...
  const columns = useMemo<ColDef[]>(() => [
    ...
    { field: 'companyName', headerName: '회사명', width: 200, editable: true },
    { field: 'category', headerName: '구분', width: 120, editable: true },
    { field: 'item', headerName: '품목', width: 500, editable: true },
  ...
  const handleBatchSave = useCallback(async (rows: { _rowState: string; [key: string]: unknown }[]) => {
    const res = await authFetch('/api/master/suppliers/batch', {
      method: 'POST',
