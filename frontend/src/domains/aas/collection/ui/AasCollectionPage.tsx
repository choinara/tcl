...
import { Upload, Merge, FileSpreadsheet, X, Copy, Download, Trash2 } from 'lucide-react';
import * as XLSX from 'xlsx';
import type { FileHeader } from '../../shared/types';
import {
  ENGINEERING_4F_TEMPLATE_ROWS,
  ENGINEERING_6F_TEMPLATE_ROWS,
  ENGINEERING_CSV_HEADER,
} from '../engineeringTemplates';
...
export default function AasCollectionPage() {
  const perm = usePermission('AA0030');
  ...
  const loadItems = useCallback(async (instanceId: number | null) => {
    setLoading(true);
    try {
      const url = instanceId != null
        ? `/api/opcua/collection-items?instanceId=${instanceId}`
        : '/api/opcua/collection-items';
  ...
  // -----------------------------------------------------------------------
  // PeakEditGrid Column Definitions
  // -----------------------------------------------------------------------

  const columns = useMemo<ColDef<CollectionItemRow>[]>(() => [
    {
      headerName: t('page.aas.collection.aasPropertyPath', 'AAS 속성 경로'),
      field: 'aas_property_path',
      width: 250,
      editable: false,
      cellClass: 'ag-left-aligned-cell',
    },
    {
      headerName: t('page.aas.collection.edgeName', 'EdgeName'),
      field: 'edge_name',
      width: 100,
      editable: true,
    },
    {
      headerName: t('page.aas.collection.equipName', 'EquipmentName'),
      field: 'equip_name',
      width: 140,
      editable: true,
    },
    {
      headerName: t('page.aas.collection.nodeId', 'NodeID'),
      field: 'node_id',
      width: 280,
    ...
    {
      headerName: t('page.aas.collection.category', '수집유형'),
      field: 'category',
      width: 110,
      editable: true,
    ...
    {
      headerName: t('page.aas.collection.memoryArea', '메모리 영역'),
      field: 'memory_area',
      width: 200,
      editable: true,
      cellEditor: 'agSelectCellEditor',
    ...
    {
      headerName: t('page.aas.collection.registerCount', '레지스터 수'),
    ...
    {
      headerName: t('page.aas.collection.bitPosition', '비트 위치'),
      field: 'bit_position',
      width: 90,
      editable: true,
      cellDataType: 'number',
    ...
    {
      headerName: t('page.aas.collection.samplingMs', '샘플링(ms)'),
      field: 'sampling_ms',
      width: 90,
      editable: true,
      cellDataType: 'number',
    ...
    {
      headerName: t('page.aas.collection.arrayIndex', 'ArrayIndex'),
      field: 'array_index',
      width: 80,
      editable: true,
      cellDataType: 'number',
    ...
    {
      headerName: t('page.aas.collection.isActive', '활성'),
      field: 'is_active',
      width: 60,
      editable: true,
  ...
  const handleBatchSave = useCallback(async (
    ...
    if (success) {
      ...
      await loadItems(selectedInstanceId);
    }
  }, [isAllMode, selectedInstanceId, notify, t, loadItems]);
  ...
  const handleTemplateDownload = useCallback((floor: '4F' | '6F') => {
    const selectedInstance = instances.find(i => i.id === selectedInstanceId);
    const equipName = selectedInstance ? selectedInstance.instanceName : 'EQUIP-NAME';

    const templateRows = floor === '4F' ? ENGINEERING_4F_TEMPLATE_ROWS : ENGINEERING_6F_TEMPLATE_ROWS;
    const header = ENGINEERING_CSV_HEADER.join(',');
    const body = templateRows
      .map(row => row.map((cell, idx) => {
        // col 2: EquipmentName, col 3: FieldDataIdentifier(NodeID) → replace EQUIP-NAME
        if (idx === 2 || idx === 3) return cell.replace(/EQUIP-NAME/g, equipName);
        return cell;
      }).join(','))
      .join('\n');
    const csv = '\uFEFF' + header + '\n' + body + '\n';
    ...
    link.href = url;
    link.download = selectedInstance
      ? `engineering_${floor}_${equipName}.csv`
      : `engineering_${floor}_template.csv`;
    link.click();
    URL.revokeObjectURL(url);
  }, [instances, selectedInstanceId]);

  // -----------------------------------------------------------------------
  // Extra Toolbar Buttons (items tab)
  // -----------------------------------------------------------------------
  ...
  const extraToolbarButtons = useMemo(() => {
    ...
    return (
      <>
        ...
        <button className="mes-btn" onClick={() => handleTemplateDownload('4F')}>
          <Download className="w-4 h-4" /> {t('page.aas.collection.templateDownload4F', '4F 템플릿')}
        </button>
        <button className="mes-btn" onClick={() => handleTemplateDownload('6F')}>
          <Download className="w-4 h-4" /> {t('page.aas.collection.templateDownload6F', '6F 템플릿')}
        </button>
        <button
          className="mes-btn"
          disabled={uploading}
  ...
  }, [perm.canCreate, handleCsvImport, handleTemplateDownload, isAllMode, notify, t, uploading]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {/* ─── Single-row header: Title + Tabs + Instance + Category ─── */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 16px', flexWrap: 'wrap', flexShrink: 0 }}>
        ...
        <button style={tabBtnStyle(activeTab === 'merge')} onClick={() => setActiveTab('merge')}>
          <Merge className="w-4 h-4" /> {t('page.aas.collection.headerMerge', '헤더 병합')}
        </button>
        ...
        {activeTab === 'items' && (
          <>
            ...
            <label style={{ fontSize: 13, fontWeight: 500, whiteSpace: 'nowrap' }}>
              {t('page.aas.collection.instanceSelect', 'Asset Instance')}:
            </label>
            <DropDown
              options={[
              ...
              {(['all', ...CATEGORIES] as const).map(cat => (
                <button
                  ...
                  style={{
                    ...
                    background: categoryFilter === cat ? 'var(--color-primary)' : 'var(--color-bg-hover)',
                    color: categoryFilter === cat ? '#fff' : 'var(--color-text-secondary)',
                    fontWeight: categoryFilter === cat ? 600 : 400,
                  }}
                >
                  {cat === 'all' ? t('common.all', '전체') : cat}
                </button>
              ))}
            </div>
        ...
        <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          {activeTab === 'items' && (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
              ...
              <PeakEditGrid
                ref={gridRef}
                gridId="aa0030-collection-items-v4"
                data={filteredItems}
                columns={columns}
                permission={perm}
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12, overflow: 'hidden' }}>
              <div style={cardStyle}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  <h3 style={{ fontWeight: 700 }}>파일 업로드</h3>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {mergeFiles.map((file, idx) => (
                      <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', background: 'var(--color-bg-hover)', borderRadius: 6, fontSize: 13 }}>
                        ...
                        <span style={{ color: 'var(--color-text-disabled)' }}>({file.headers.length}열)</span>
              ...
              {mergeFiles.length > 0 && (
                <div style={{ ...cardStyle, flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    <h3 style={{ fontWeight: 700 }}>헤더 비교 ({mergeFiles.length}개 파일)</h3>
                    <button className="mes-btn mes-btn-save" onClick={doMergeHeaders}>
                      <Merge className="w-4 h-4" /> 중복 제거 및 병합
                    </button>
                  </div>
                  <div style={{ flex: 1, overflow: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr>
                          <th style={{ ...tableHeaderStyle, position: 'sticky', left: 0, zIndex: 10, minWidth: 150 }}>파일명</th>
                          {Array.from({ length: maxMergeColumns }, (_, i) => (
                            <th key={i} style={{ ...tableHeaderStyle, textAlign: 'center', minWidth: 120 }}>구분{i + 1}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                ...
                <div style={{ ...cardStyle, border: '1px solid #86efac' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    <h3 style={{ fontWeight: 700, color: '#16a34a' }}>
                      병합 결과 ({mergeFiles.flatMap(f => f.headers).length} -&gt; {mergedHeaders.length}개)
                    </h3>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button className="mes-btn" onClick={copyMergedHeaders}>
                        <Copy className="w-4 h-4" /> {copied ? '복사됨!' : '클립보드 복사'}
                      </button>
                  ...
                  <div style={{ overflow: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr>{mergedHeaders.map((_, i) => <th key={i} style={{ ...tableHeaderStyle, textAlign: 'center', minWidth: 120, background: '#dcfce7' }}>구분{i + 1}</th>)}</tr>
                      </thead>
                      <tbody>
        ...
        >
          ...
          <div style={{ maxHeight: 300, overflow: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  <th style={{ ...tableHeaderStyle, width: 60 }}>행</th>
                  <th style={tableHeaderStyle}>사유</th>
                </tr>
              </thead>
              <tbody>
